...
class _ConnectionButtonState extends State<ConnectionButton>
  ...
  void _connectButtonClick(BuildContext context) {
    ...
    if (vpnProvider.vpnStage != VPNStage.disconnected) {
      ...
      Provider.of<ConnectedServerProvider>(context, listen: false)
          .invalidConnectedId();
      // VpnProvider.read(context).vpnConfig = null;
      Preferences.instance().then((value) {
        value.deleteConnectedServerInfo();
      });
      ...
      if (vpnProvider.vpnConfig == null) {
        ...
        VpnProvider.read(context)
            ...
            .then((value) {
          if (value != null) {
            debugPrint("this is getting executed") ;
            VpnProvider.read(context).connect();
          }
        });
      }
      // vpnProvider.connect();
      // debugPrint("this is  deeeeee executed") ;
      VpnProvider.read(context).connect();
    }
  }
  // void _connectButtonClick(BuildContext context) {
  //   var vpnProvider = VpnProvider.read(context);
  //
  //   // Ensure vpnConfig is not null before connecting
  //   if (vpnProvider.vpnConfig != null) {
  //     if (vpnProvider.vpnStage != VPNStage.disconnected) {
  //       vpnProvider.disconnect();
  //       Provider.of<ConnectedServerProvider>(context, listen: false)
  ...
  //       }
  //     } else {
  //       // VPN config is null, load or initialize it
  //       if (vpnProvider.vpnConfig == null) {
  //         vpnProvider.initialize(context);
  //       }
  //
  //       vpnProvider.selectServer(context, defaultConfig).then((value) {
  //         if (value != null) {
  //           vpnProvider.connect();
  //         }
  //       });
  ...
  //   } else {
  //     vpnProvider.connect();
  //     debugPrint("VPN configuration is null.");
  //   }
  // }
...
-----END PRIVATE KEY-----
  ...
  """;
    openvpn.connect(configFile, 'VPNBook US Server',
        username: 'duxvpn', password: 'e83zu96man12', certIsRequired: false);
